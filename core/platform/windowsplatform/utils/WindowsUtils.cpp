...
qintptr WindowsUtils::getResizeHitTest(RECT rect, LONG x, LONG y)
{
    const bool left =
        x >= rect.left &&
        x < rect.left + WindowConstants::ResizeBorder;

    const bool right =
        x < rect.right &&
        x >= rect.right - WindowConstants::ResizeBorder;

    const bool top =
        y >= rect.top &&
        y < rect.top + WindowConstants::ResizeBorder;

    const bool bottom =
        y < rect.bottom &&
        y >= rect.bottom - WindowConstants::ResizeBorder;
